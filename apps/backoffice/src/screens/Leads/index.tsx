...
const Leads = () => {
  ...
  return (
    <Layout>
      <HeaderPage title="Leads">
        <Breadcrumbs items={["Realty", "Leads"]} />
      </HeaderPage>
      <div className={styles.header_page}>
        <Field>
          <Input type="search" placeholder="Search lead" onChange={({ target }) => setSearch(target.value)} />
        </Field>
        <span>
      ...
      {loading ? (
       ...
       <>
        ...
        <Table  
        count={result.count}
        data={result.items.filter((item:any) => item.uuid == 're')}
        setLimit={setLimit}
        limit={limit}
        setPage={setPage}
        headers={['Name', 'Email', 'Country', 'Phone']}
        ...
        <EmptyTable title='Leads' url='/leads'  />
      )}
       </> 
      )}
